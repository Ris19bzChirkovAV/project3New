...
public class bomb9 : MonoBehaviour
{
    ...
    private void Start()
    {
        //boomRadius = PlayerPrefs.GetFloat("Bomb1Radius");
    }
    public void goBoom()
    {
        StartCoroutine(Boom());
    ...
    public IEnumerator Boom()
    {
        ...
        Collider2D[] colliders = Physics2D.OverlapCircleAll(transform.position, 12);
        foreach (Collider2D C in colliders)
        {
            if (C.tag == "Kamen")
            {
                ...
                C.gameObject.GetComponent<Rigidbody2D>().AddForce(new Vector3(mulX * -500, mulY * -500, 0));
